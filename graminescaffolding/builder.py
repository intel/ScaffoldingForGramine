...
class Builder:
    ...
    def __init__(self, project_dir, config):
        ...
        self.relative_magic_dir = self.project_dir / SCAG_MAGIC_DIR
        self.rootfs_tar = self.relative_magic_dir / 'rootfs.tar'
    ...
    def render_templates(self):
        ...
        for path, template_names in want_files.items():
            self._render_template_to_path(
                ...
                self.relative_magic_dir / path)


    def render_client_config(self, mrenclave):
        self._render_template_to_path(
            'scag-client.toml',
            self.relative_magic_dir / 'scag-client.toml',
            mrenclave=mrenclave)
    ...
    def create_chroot(self):
        ...
        subprocess.run([
            ...
            '--setup-hook',
                f'sh {self.relative_magic_dir / "mmdebstrap-hooks/setup.sh"} "$@"',
            '--customize-hook',
                f'sh {self.relative_magic_dir / "mmdebstrap-hooks/customize.sh"} "$@"',

            CODENAME,
            self.rootfs_tar,
            self.relative_magic_dir / 'sources.list',
        ], check=True)
    ...
    def sign_docker_image(self, image):
        ...
        (self.relative_magic_dir / 'app.manifest.sgx').write_bytes(msgx)
        (self.relative_magic_dir / 'app.sig').write_bytes(sig)
