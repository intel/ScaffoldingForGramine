...
{% block install -%}
cp -rT {{ scag.builder.project_dir }} "$1"/app
rm -rf "$1"/app/.scag
cp -rT {{ scag.builder.project_dir }}/.scag/etc "$1"/usr/local/etc
cp -r {{ scag.builder.project_dir }}/.scag/app.manifest.template "$1"/app
{%- endblock %}
